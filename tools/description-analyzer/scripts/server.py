#!/usr/bin/env python3
"""
Mini HTTP server for the Field Description Analyzer tool.
"""
import json
import sys
import os
...
class DescriptionAnalyzerHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/api/analyze':
            ...
            try:
                ...
                self._json_response({'csv': csv_content, 'stats': stats})
...
if __name__ == '__main__':
    os.chdir(PUBLIC_DIR)
    server = HTTPServer(('', PORT), DescriptionAnalyzerHandler)
    print(f"📝 Description Analyzer running on http://localhost:{PORT}")
    try:
